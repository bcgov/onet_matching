...
So obviously this is a ridiculously unrealistic intervention: there would be mass upheaval if `r scales::percent(as.vector(table(noc_change$unchanged)/nrow(noc_change))[1], accuracy=1)` of the labour market changed occupations.  But still, what would be the impact on employment income if the government matched workers with jobs to minimize the average distance?  To do so we take the model that was fit on the training data, and make two predictions: one based actual occupations (and distances), and one based on the intervention.  First off, summary statistics:
